...
rulesHigh<-apriori(surveyDataX ,parameter = list(support=0.0005,confidence=0.5,minlen=4,maxtime=0,maxlen=5),appearance = list(rhs=c("Satisfaction=High")))
# Got 55 rules
...
outboundMap<-map_simple+
  geom_curve(aes(x=LonW,y=LatW,xend=ConWithIllinois$lon ,yend=ConWithIllinois$lat, 
                 color=Sat)
             data=ConWithIllinois)
outboundMap
